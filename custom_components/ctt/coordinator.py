...
from .api import CTTApiClient, CTTApiError
from .const import (
...
class CTTCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (CTTApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if (
                    isinstance(result, CTTApiError)
        ...
        if saw_429:
            ...
            raise UpdateFailed(
                "CTT rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes and errors == len(codes) and not raws:
            raise UpdateFailed("CTT unreachable for all tracked parcels")
