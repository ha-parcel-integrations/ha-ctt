"""Calendar platform for the CTT parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the CTT deliveries calendar from a config entry."""
    async_add_entities([CTTDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class CTTDeliveriesCalendar(CoordinatorEntity[CTTCoordinator], CalendarEntity):
    """A read-only calendar of expected CTT deliveries.
    ...
    def __init__(self, coordinator: CTTCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "CTT parcel")
            description_parts = [
