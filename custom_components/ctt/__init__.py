"""CTT parcel tracker custom component for Home Assistant."""
...
@dataclass
class CTTData:
    """Runtime data attached to the CTT config entry."""

    client: CTTApiClient
    coordinator: CTTCoordinator


type CTTConfigEntry = ConfigEntry[CTTData]


async def async_setup_entry(hass: HomeAssistant, entry: CTTConfigEntry) -> bool:
    """Set up CTT from a config entry."""
    # No auth: CTT tracking is public, so the HA-managed session is fine.
    client = CTTApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = CTTData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: CTTConfigEntry) -> bool:
    """Unload the CTT config entry."""
