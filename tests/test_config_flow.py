"""Tests for the CTT config and options flow."""

from pytest_homeassistant_custom_component.common import MockConfigEntry

from custom_components.ctt.config_flow import (
...
def test_normalize_tracking_code_strips_and_uppercases():
    assert normalize_tracking_code("rr 999999999-pt") == "RR999999999PT"
...
async def test_options_add_parcel(hass):
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"tracking_codes": ["rr999999999pt"]}
    )
    assert result["type"] == "create_entry"
...
async def test_options_add_code_with_separators(hass):
    ...
    result = await _open_options_step(hass, entry, "parcels")
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"tracking_codes": ["rr-999999999-pt"]}
    )
    assert result["type"] == "create_entry"
    assert result["data"][CONF_PARCELS] == [{CONF_TRACKING_CODE: "RR999999999PT"}]
...
async def test_options_de_duplicates_tracking_codes(hass):
    entry = _hub([{CONF_TRACKING_CODE: "RR111111111PT"}])
    entry.add_to_hass(hass)
    result = await _open_options_step(hass, entry, "parcels")
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"tracking_codes": ["RR111111111PT", "rr111111111pt"]}
    )
    assert result["data"][CONF_PARCELS] == [{CONF_TRACKING_CODE: "RR111111111PT"}]


async def test_options_remove_parcel(hass):
    entry = _hub(
        [
            {CONF_TRACKING_CODE: "RR111111111PT"},
            {CONF_TRACKING_CODE: "RR222222222PT"},
        ]
    )
    entry.add_to_hass(hass)
    result = await _open_options_step(hass, entry, "parcels")
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"tracking_codes": ["RR222222222PT"]}
    )
    assert result["type"] == "create_entry"
    codes = {p[CONF_TRACKING_CODE] for p in result["data"][CONF_PARCELS]}
    assert codes == {"RR222222222PT"}
