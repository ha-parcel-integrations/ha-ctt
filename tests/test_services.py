"""Tests for the CTT services (track_parcel / untrack_parcel)."""
...
async def test_track_parcel_adds_to_options(hass):
    ...
    with patch(
        ...
        await hass.services.async_call(
            DOMAIN,
            "track_parcel",
            {CONF_TRACKING_CODE: "RR999999999PT"},
            blocking=True,
        )
        await hass.async_block_till_done()

    parcels = entry.options[CONF_PARCELS]
    assert parcels == [{CONF_TRACKING_CODE: "RR999999999PT"}]
...
async def test_track_parcel_normalizes_code(hass):
    ...
    with patch(
        ...
        await hass.services.async_call(
            DOMAIN,
            "track_parcel",
            {CONF_TRACKING_CODE: "rr-999 999 999-pt"},
            blocking=True,
        )
        await hass.async_block_till_done()
...
async def test_untrack_parcel_removes_from_options(hass):
    entry = await _setup(
        hass, parcels=[{CONF_TRACKING_CODE: "RR999999999PT"}]
    )
    with patch(
        "custom_components.ctt.api.CTTApiClient.async_get_parcel",
        new=AsyncMock(return_value=_SAMPLE),
...
async def test_untrack_unknown_code_is_noop(hass):
    ...
    with patch(
        ...
        await hass.services.async_call(
            DOMAIN,
            "untrack_parcel",
            {CONF_TRACKING_CODE: "RR000000000PT"},
            blocking=True,
        )
        await hass.async_block_till_done()
