"""Tests for CTT sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.ctt.const import ParcelStatus
from custom_components.ctt.sensor import (
    CTTAwaitingPickupSensor,
    CTTDeliveredParcelsSensor,
    CTTEnRouteToPickupPointSensor,
    CTTIncomingParcelsSensor,
    CTTLastUpdateSensor,
    CTTNextDeliverySensor,
    CTTParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = CTTIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = CTTParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = CTTParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = CTTNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = CTTNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_en_route_and_awaiting_pickup_split():
    ...
    en_route = CTTEnRouteToPickupPointSensor(coordinator, _entry())
    awaiting = CTTAwaitingPickupSensor(coordinator, _entry())
...
def test_last_update_sensor():
    ...
    sensor = CTTLastUpdateSensor(coordinator, _entry())
