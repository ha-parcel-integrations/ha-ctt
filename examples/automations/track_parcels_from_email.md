# Automatic parcel tracking from e-mail (IMAP → CTT)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the CTT tracking code, and register it with `ctt.track_parcel` — fully automatic, no extra custom component required.

CTT is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `ctt.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ ctt.track_parcel
                                     │
...
- This integration, with the `ctt.track_parcel` action available (field `tracking_code`).
...
The regex matches CTT's own UPU S10 shape directly — two letters, nine digits, two letters:

```
(?<![0-9A-Za-z])[A-Z]{2}[0-9]{9}[A-Z]{2}(?![0-9A-Za-z])
```

This is the same pattern the integration's own config flow validates
against (`custom_components/ctt/config_flow.py`), so anything the automation
matches is also something `ctt.track_parcel` will accept. All four real
codes seen during research fit this shape — including a **German**-issued
one CTT still tracks, so the pattern deliberately does not pin the trailing
country letters to `PT`. If a genuine CTT code ever shows up in a different
shape, the AI fallback will still surface it; widen the regex once you have
a confirmed example rather than guessing ahead of one.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@ctt.pt","subject":"O seu envio está a caminho",
       "text":"Código de rastreio: RR123456785PT","initial":true,"folder":"INBOX","username":"test"}'
```

Then `ctt.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
